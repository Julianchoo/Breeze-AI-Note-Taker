...
import { CHUNK_SECONDS, MAX_MEETING_SECONDS } from "./meeting-types";
...
/** Splits into CHUNK_SECONDS parts; only the last may be shorter (the server requires this). */
export function sliceChunks(samples: Int16Array): Int16Array[] {
  const chunks: Int16Array[] = [];
  for (let start = 0; start < samples.length; start += CHUNK_SAMPLES)
    chunks.push(samples.subarray(start, start + CHUNK_SAMPLES));
  return chunks;
}
