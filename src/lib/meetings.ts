...
export async function processMeeting(id: string, userId: string) {
  ...
  try {
    ...
    if (chunk) {
      ...
      const offset = chunks.reduce((n, c) => c.index < chunk.index ? n + c.durationSeconds : n, 0);
      segments = result.segments.map(s => ({ start: offset + Math.min(s.start, chunk.durationSeconds), end: offset + Math.min(Math.max(s.end, s.start), chunk.durationSeconds), text: s.text, speaker: names.get(s.speaker ?? "unknown")! }));
    } else {
